...
🎁 ��ри заказе сайта ДЖАРВИС станет вашим персональным AI-помощником без ограничений!`
    })
  }
  ...
  try {
    ...
    if (!openRouterApiKey) {
      ...
      // Greeting responses
      if (lastMessage.includes('привет') || lastMessage.includes('здравс��вуй') || lastMessage.includes('добро пожаловать') || messages.length === 1) {
...
• Персонализация пользо��ательского опыта
