...
// Хранилище лим��тов в памяти (в production лучше использовать Redis)
  ...
  try {
    ...
    if (!openRouterApiKey) {
      ...
      if (lastMessage.includes('привет') || lastMessage.includes('здравствуй') || lastMessage.includes('добро пожаловать') || messages.length === 1) {
        response = `Привет! Я ДЖАРВИС, ваш AI-помощни�� по веб-разработке! 🚀
...
Хотите увидеть ��емо или обсудить ваш проект?`
      }
      // Creator questions
...
Могу помочь с:
• Техническими вопросами по разр��ботке
...
Отвечай на русском языке. Будь максимально полезным и информативным!`
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        ...
        return res.status(200).json({
          message: 'Привет! Я ДЖАРВИС. Сейчас у меня техничес��ие проблемы с подключением к AI-сервису, но я все��да готов помочь! 🤖\n\nОбратитесь ко мне в Telegram @jarvis_ai_dev - там я отвечу на любые вопросы по веб-разработке и AI!'
        })
